...
static void GUI_Refresh(const HWND hW, const int revertbtn)
{
	...
	if(ONLY1PLAYERACTIVE) // we accept all device input if there is only one player active, so disable all device related comboboxes/button
	{
		...
		// hide select combobox/text
		ShowWindow(GetDlgItem(hW, IDC_MOUSESELECTINFO), 0);
		ShowWindow(GetDlgItem(hW, IDC_MOUSESELECT), 0);
		ShowWindow(GetDlgItem(hW, IDC_KEYBOARDSELECTINFO), 0);
		ShowWindow(GetDlgItem(hW, IDC_KEYBOARDSELECT), 0);
	}
	else // multiple players are active
	{
		const int showdevices = PROFILE[currentplayer].SETTINGS[CONFIG] != DISABLED && DEV_Init() > 2; // if profile is active and multiple devices connected, show detect devices button and combo boxes
		ShowWindow(GetDlgItem(hW, IDC_DETECTDEVICE), showdevices); 
		EnableWindow(GetDlgItem(hW, IDC_DETECTDEVICE), showdevices);
		EnableWindow(GetDlgItem(hW, IDC_MOUSESELECT), showdevices);
		EnableWindow(GetDlgItem(hW, IDC_KEYBOARDSELECT), showdevices);
		// show select combobox/text
		ShowWindow(GetDlgItem(hW, IDC_MOUSESELECTINFO), showdevices);
		ShowWindow(GetDlgItem(hW, IDC_MOUSESELECT), showdevices);
		ShowWindow(GetDlgItem(hW, IDC_KEYBOARDSELECTINFO), showdevices);
		ShowWindow(GetDlgItem(hW, IDC_KEYBOARDSELECT), showdevices);
	}
